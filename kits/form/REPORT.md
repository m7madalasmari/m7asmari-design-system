...
- **أهم 3 تقويات core قبل التوزيع كحزمة:**
  1. توكنة حلقات الخطأ/النجاح (`--input-ring-error/-success`) بدل `rgba()` الخام.
  2. توسيع `contrast.py` ليشمل أزواج رسائل الحقول والplaceholder تلقائيًا.
  3. تدقيق `:focus-visible` للعناصر ذات الأدوار المخصّصة (checkbox/switch/select) ضمن `audit`.
